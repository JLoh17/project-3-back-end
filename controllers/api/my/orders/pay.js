...
const apiCreateNewPaidOrder = async function(req, res) {
  ...
  // const cart = await currentUser.getOrder()
  // alternative writing of above line
  const cart = await Order.findOne({ where: { UserId: currentUser.id }})
  ...
  await Order.create({
    ...
    // takes out status and maps the status to "Pending Delivery"
    status: 'Pending-Delivery',
    OrderProduct: orderProductData
  }, {
    fields: permittedParams,
    include: {
      association: Order.OrderProduct,
    },
  })
  ...
  res.json({ message: 'Created new Paid Order'})
}
